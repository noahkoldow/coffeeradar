...
export type HistoryState = {
  ...
  /** Track when we last asked to convert an activity into a habit to avoid prompt spam. */
  habitPromptedAtByActivityId?: Record<string, string>; // activityId/titleKey -> ISO timestamp
};
...
export type LocationProfile = {
  ...
  detectedAt: string;
  /** Learned home anchor used to detect when the user is likely traveling. */
  homeBase?: {
    lat: number;
    lng: number;
  ...
  /** Temporary travel context that can bias suggestions at new locations. */
  travelContext?: {
    active: boolean;
    inferredPurpose: 'sightseeing' | 'business' | 'unknown';
    startedAt: string;
    expiresAt: string;
